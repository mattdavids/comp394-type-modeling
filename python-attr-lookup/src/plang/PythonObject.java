...
public class PythonObject {
    ...
    protected List<PythonObject> buildMRO() {
        ...
        mro.add(this);
        if(type != null) {
            List<PythonObject> type_mro = type.getMRO();
            mro.addAll(type_mro);
        }
        return mro;
    }
...
//        So, originally this looked really nice and was just
//        if(this.getMRO().size() > 1) {
//            return mro.get(1).get(attrName);
//        }
//        but unfortunately, this made it get the lowest level's type
//        which is incorrect :( So now it's all of this
        this.getMRO();
        if (mro.size() == 0) {
            return new PythonObject(null);
        }
        for (PythonObject obj: mro.subList(1, mro.size())) {
            try {
            PythonObject item = obj.get(attrName);
            if(item != new PythonObject(null)) {
                return item;
            }
